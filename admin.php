...
try {
    ...
    // Total cases
    $stmt = $conn->query("SELECT COUNT(*) as total_cases FROM cases");
    $total_cases = $stmt->fetch(PDO::FETCH_ASSOC)['total_cases'];
    
    // Total services
    $stmt = $conn->query("SELECT COUNT(*) as total_services FROM services");
    $total_services = $stmt->fetch(PDO::FETCH_ASSOC)['total_services'];
    
    // Total contacts
    $stmt = $conn->query("SELECT COUNT(*) as total_contacts FROM contacts");
    $total_contacts = $stmt->fetch(PDO::FETCH_ASSOC)['total_contacts'];
    
} catch (Exception $e) {
    $total_users = $total_cases = $total_services = $total_contacts = 0;
}
?>
<!DOCTYPE html>
<html lang="en">
                ...
                <div class="stats-card">
                    ...
                    <div class="mt-2">
                        <small class="text-warning">
                            <i class="fas fa-clock me-1"></i> 15 pending
                        </small>
                    </div>
                </div>
            </div>
                ...
                <div class="stats-card">
                    ...
                    <div class="mt-2">
                        <small class="text-info">
                            <i class="fas fa-star me-1"></i> 3 featured
                        </small>
                    </div>
                </div>
            </div>
